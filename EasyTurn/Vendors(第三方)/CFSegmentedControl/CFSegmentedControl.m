...
- (void)setDataSource:(id<CFSegmentedControlDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:kACColorClear andPosition:bgLayerPosition];
        [self.layer addSublayer:bgLayer];
        [tempBgLayers addObject:bgLayer];
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval , self.frame.size.height / 2);
        NSString *titleString = [_dataSource getSegmentedControlTitles][i];
        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:kACColorBlack andPosition:titlePosition];
...
- (void)animateTitle:(CATextLayer *)title isSelect:(BOOL)isSelect complete:(void(^)(void))complete {
    ...
    {
        title.foregroundColor = kACColorBlack.CGColor;
    }
