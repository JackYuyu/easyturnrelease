...
@interface ETServiceDetailController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation ETServiceDetailController

+ (instancetype)serviceDetailController:(NSDictionary *)detailInfo{
    ETServiceDetailController *vc = [[self alloc] init];
...
- (void)viewDidLoad {
    ...
    self.tableView.tableHeaderView = [ETSaleDetailHeaderView serviceDetailHeaderView:self.detailInfo click:^{
...
//                int a=[EaseTextView numberOfLinesForMessage:self.detailInfo[@"business"]]+1;
            ...
            case 1:
            {
                [dict setObject:@"所属类型" forKey:@"title"];
