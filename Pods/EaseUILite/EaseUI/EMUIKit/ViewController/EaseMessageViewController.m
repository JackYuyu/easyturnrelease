...
//        _releaseid=[d objectForKey:@"uid"];
    ...
    NSString* a=conv.conversationId;
    _touser=a;
    [self getavatr];
...
//        if ([from.cart isEqualToString:@"1"]) {
//            if (!_cartcontroller) {
//                [self createButton];
//            }
//        }
...
- (void)createButton{
    ...
    _button.frame = CGRectMake(self.view.bounds.size.width -80, self.view.bounds.size.height - 150, 40, 40);
    ...
    _mbutton.frame = CGRectMake(self.view.bounds.size.width -80 -_button.frame.size.width-10-50, self.view.bounds.size.height - 150+5, 100, 30);
    
    _mbutton.titleLabel.font = [UIFont systemFontOfSize:12.0f];
    
    [_mbutton setBackgroundColor:[UIColor colorWithRed:137 green:137 blue:137 alpha:1.0]];
...
//    NSArray*arr=[EaseUserModel bg_f];
    EaseUserModel* em=eu[0];
//    self.title=em.nickname;
    model.avatarURLPath=em.avatarURLPath;
...
//        }
        ...
        model.nickname=_touserNick;
        sendCell.model=model;
        EaseUserModel* user=[EaseUserModel new];
        user.buddy=_touser;
        user.nickname=_touserNick;
        user.avatarURLPath=_touserAvat;
