...
- (void)_setupDetailLabelConstraints
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.detailLabel attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeRight multiplier:1.0 constant:-80]];
...
- (void)setModel:(id<IConversationModel>)model
{
    ...
    if (array1.count>0) {
        ...
        self.titleLabel.text=from.nickname;
    }
    if (self.showAvatar) {
        if (_model.avatarURLPath && ![_model.avatarURLPath isKindOfClass:[NSNull class]]) {
...
//            model.nickname=from.nickname;
        ...
        [self.avatarView setImageCornerRadius:20];
